...
include 'C:\moodel\server\moodle\question\type\truefalse\edit_truefalse_form.php';
...
class qtype_multichoice_edit_form extends question_edit_form {
     ...
     */
    private function truefalse ($form)
    {
        
          $form->addElement('select', 'correctanswer',
                get_string('correctanswer', 'qtype_truefalse'), array(
                0 => get_string('false', 'qtype_truefalse'),
                1 => get_string('true', 'qtype_truefalse')));
          $form->addElement('editor', 'feedbacktrue',
                get_string('feedbacktrue', 'qtype_truefalse'), array('rows' => 10), $this->editoroptions);
        $form->setType('feedbacktrue', PARAM_RAW);

        $form->addElement('editor', 'feedbackfalse',
                get_string('feedbackfalse', 'qtype_truefalse'), array('rows' => 10), $this->editoroptions);
        $form->setType('feedbackfalse', PARAM_RAW);

        $form->addElement('header', 'multitriesheader',
                get_string('settingsformultipletries', 'question'));

        $form->addElement('hidden', 'penalty', 1);
        $form->setType('penalty', PARAM_FLOAT);

        $form->addElement('static', 'penaltymessage',
                get_string('penaltyforeachincorrecttry', 'question'), 1);
        $form->addHelpButton('penaltymessage', 'penaltyforeachincorrecttry', 'question');
    }
    
    protected function definition_inner($mform) {
       $this->truefalse($mform);
        ...
        $this->add_per_answer_fields($mform, get_string('choiceno', 'qtype_multichoice', '{no}'),
                question_bank::fraction_options_full(), max(2, QUESTION_NUMANS_START));
